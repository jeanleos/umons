...
architecture behave of scale_CTRL_s_axi is
    ...
    constant ADDR_SCALE_FACTOR_DATA_0 : INTEGER := 16#18#;
    constant ADDR_SCALE_FACTOR_CTRL   : INTEGER := 16#1c#;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SCALE_FACTOR_DATA_0 =>
                        rdata_data <= RESIZE(int_scale_factor(31 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_scale_factor(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SCALE_FACTOR_DATA_0) then
                    int_scale_factor(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_scale_factor(31 downto 0));
                end if;
            end if;
        end if;
    end process;
